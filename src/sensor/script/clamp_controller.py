...
class SemanticRelayController:
    def __init__(self):
        rospy.init_node("clamp_controller", anonymous=False)

        self.enable_topic = rospy.get_param("~enable_topic", "/clamp/enable")
        self.service_name = rospy.get_param("~service_name", "/relay_driver/output2/set")
        self.label = rospy.get_param("~label", "clamp")
