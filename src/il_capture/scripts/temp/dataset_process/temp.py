...
def process_point_cloud_data(pc_data, T_corr, target_points=2048):
    ...
    for i in range(num_frames):
        ...
        # 2. 过滤 Z < 0
        mask = xyz_corrected[:, 2] >= -0.05
        xyz_filtered = xyz_corrected[mask]
        feats_filtered = feats[mask]
        mask_2 = xyz_filtered[:, 2] <= 0.3
        xyz_filtered = xyz_filtered[mask_2]
        feats_filtered = feats_filtered[mask_2]
...
X_OFFSET = -0.015
Z_OFFSET = 0.015
def adjust_xz(arr, x_col=0, z_col=2):
    """对数组的 x/z 列做偏移，保持形状不变"""
    if arr is None:
        return arr
    if arr.ndim < 2 or arr.shape[1] <= max(x_col, z_col):
        return arr
    arr = arr.copy()
    arr[:, x_col] += X_OFFSET
    arr[:, z_col] += Z_OFFSET
        ...
        for demo_name in tqdm.tqdm(demo_keys):
            ...
            for key in src_demo.keys():
                if key != 'obs':
                    ...
                    if key in ['action_with_wrench', 'action_without_wrench']:
                        data = src_demo[key][:]
                        data = adjust_xz(data, x_col=0, z_col=2)
                        dst_demo.create_dataset(key, data=data)
                    else:
                    ...
                    for obs_key in src_obs.keys():
                        if obs_key == 'robot0_eef_pos':
                            ...
                            data = adjust_xz(data, x_col=0, z_col=2)
                            dst_obs.create_dataset(obs_key, data=data)
                        elif obs_key != 'pointcloud':
